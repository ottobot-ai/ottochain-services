 ...
 * 
 * Version info is loaded from .env.version (generated by scripts/build-version.sh)
 */

const fs = require('fs');
const path = require('path');

// Load version info from .env.version if it exists
const versionEnvPath = path.join(__dirname, '.env.version');
if (fs.existsSync(versionEnvPath)) {
  const versionEnv = fs.readFileSync(versionEnvPath, 'utf8');
  for (const line of versionEnv.split('\n')) {
...
const sharedEnv = {
  ...
  // Version info (from .env.version, generated at build time)
