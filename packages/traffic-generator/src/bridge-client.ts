...
/**
 * Token creation parameters (maps to POST /token/create).
 * behavior is 0–15 encoding the TDEG (Transferable, Divisible, Expirable, Governable) matrix.
 */
export interface CreateTokenParams {
  ...
  /** Optional pre-set tokenId (UUID). Generated by bridge if omitted. */
  tokenId?: string;
}
...
export class BridgeClient {
  ...
  // ==========================================================================
  // Token Domain (POST /token/*)
  // ==========================================================================

  /**
   * Mint a new token fiber using the TDEG 16-type behavior model.
   * Maps to POST /token/create.
   */
  async createToken(params: CreateTokenParams): Promise<TokenCreateResponse> {
    return this.post<TokenCreateResponse>('/token/create', {
  ...
  /**
   * Transfer tokens to a recipient (only valid for Transferable behaviors).
   * Maps to POST /token/transfer.
   */
  async transferToken(
    privateKey: string,
    tokenId: string,
    ...
    return this.post<TransitionResponse>('/token/transfer', {
      privateKey,
      tokenId,
      recipient,
  ...
  /**
   * Burn a token, moving it to BURNED final state.
   * Maps to POST /token/burn.
   */
  async burnToken(
    privateKey: string,
    tokenId: string
  ): Promise<TransitionResponse> {
    return this.post<TransitionResponse>('/token/burn', {
      privateKey,
      tokenId,
    });
  }
  ...
  /**
   * Split a divisible token into two (Divisible behavior only).
   * Maps to POST /token/split.
   */
  async splitToken(
    privateKey: string,
    tokenId: string,
    ...
    return this.post<TransitionResponse>('/token/split', {
      privateKey,
      tokenId,
      amount,
    });
   ...
   * Maps to GET /token/:tokenId.
   */
  async getToken(tokenId: string): Promise<TokenState> {
    return this.get<TokenState>(`/token/${tokenId}`);
  }
