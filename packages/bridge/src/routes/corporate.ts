...
// ============================================================================
// Embedded State Machine Definitions
// ============================================================================

const CORPORATE_ENTITY_DEFINITION = {
  states: {
    INCORPORATING: { id: { value: 'INCORPORATING' }, isFinal: false },
    ACTIVE: { id: { value: 'ACTIVE' }, isFinal: false },
    SUSPENDED: { id: { value: 'SUSPENDED' }, isFinal: false },
    DISSOLVED: { id: { value: 'DISSOLVED' }, isFinal: true },
  },
  initialState: { value: 'INCORPORATING' },
  transitions: [
    {
      from: { value: 'INCORPORATING' },
      to: { value: 'ACTIVE' },
  ...
  metadata: { name: 'CorporateEntity', version: '1.0.0' },
};

const CORPORATE_BOARD_DEFINITION = {
  states: {
    ACTIVE: { id: { value: 'ACTIVE' }, isFinal: false },
    IN_MEETING: { id: { value: 'IN_MEETING' }, isFinal: false },
    QUORUM_LOST: { id: { value: 'QUORUM_LOST' }, isFinal: false },
  },
  ...
  metadata: { name: 'CorporateBoard', version: '1.0.0' },
};

const CORPORATE_SHAREHOLDERS_DEFINITION = {
  states: {
    SCHEDULED: { id: { value: 'SCHEDULED' }, isFinal: false },
    RECORD_DATE_SET: { id: { value: 'RECORD_DATE_SET' }, isFinal: false },
    PROXY_PERIOD: { id: { value: 'PROXY_PERIOD' }, isFinal: false },
    IN_SESSION: { id: { value: 'IN_SESSION' }, isFinal: false },
    VOTING: { id: { value: 'VOTING' }, isFinal: false },
    CLOSED: { id: { value: 'CLOSED' }, isFinal: true },
  },
  initialState: { value: 'SCHEDULED' },
  transitions: [
    {
      from: { value: 'SCHEDULED' },
      to: { value: 'RECORD_DATE_SET' },
    ...
    {
      from: { value: 'RECORD_DATE_SET' },
      to: { value: 'PROXY_PERIOD' },
    ...
    {
      from: { value: 'PROXY_PERIOD' },
      to: { value: 'IN_SESSION' },
      eventName: 'open_meeting',
      guard: { '>=': [{ var: 'event.initialQuorumCount' }, { var: 'state.quorumRequirements.sharesRequired' }] },
      effect: {
        merge: [
          { var: 'state' },
          {
  ...
  metadata: { name: 'ShareholdersMeeting', version: '1.0.0' },
};

const CORPORATE_OFFICERS_DEFINITION = {
  states: {
    ACTIVE: { id: { value: 'ACTIVE' }, isFinal: false },
  },
  initialState: { value: 'ACTIVE' },
          ...
          { var: 'state' },
          {
            officers: {
              cat: [
                { var: 'state.officers' },
                [{
                  officerId: { var: 'event.officerId' },
                  personId: { var: 'event.personId' },
                  name: { var: 'event.name' },
                  title: { var: 'event.title' },
                  customTitle: { var: 'event.customTitle' },
                  status: { if: [{ var: 'event.isInterim' }, 'INTERIM', 'ACTIVE'] },
                  appointedDate: { var: 'event.appointedDate' },
                  appointmentResolutionRef: { var: 'event.appointmentResolutionRef' },
                  reportsTo: { var: 'event.reportsTo' },
                  authorityLevel: { var: 'event.authorityLevel' },
                  spendingLimit: { var: 'event.spendingLimit' },
          ...
          { var: 'state' },
          {
            officers: {
              map: [
                ...
                {
                  if: [
                    { '===': [{ var: 'item.officerId' }, { var: 'event.officerId' }] },
                    {
                      merge: [
                        { var: 'item' },
                        {
                          status: 'REMOVED',
                          terminationDate: { var: 'event.effectiveDate' },
                        },
                      ],
                    },
                    { var: 'item' },
  ...
  metadata: { name: 'CorporateOfficers', version: '1.0.0' },
};

const CORPORATE_SECURITIES_DEFINITION = {
  states: {
    AUTHORIZED: { id: { value: 'AUTHORIZED' }, isFinal: false },
    ISSUED: { id: { value: 'ISSUED' }, isFinal: false },
    TREASURY: { id: { value: 'TREASURY' }, isFinal: false },
    TRANSFERRED: { id: { value: 'TRANSFERRED' }, isFinal: false },
    RETIRED: { id: { value: 'RETIRED' }, isFinal: true },
  },
  initialState: { value: 'AUTHORIZED' },
  transitions: [
    {
      from: { value: 'AUTHORIZED' },
      to: { value: 'ISSUED' },
  ...
  metadata: { name: 'CorporateSecurities', version: '1.0.0' },
};

const CORPORATE_COMPLIANCE_DEFINITION = {
  states: {
    COMPLIANT: { id: { value: 'COMPLIANT' }, isFinal: false },
    REVIEW_PENDING: { id: { value: 'REVIEW_PENDING' }, isFinal: false },
    DEFICIENT: { id: { value: 'DEFICIENT' }, isFinal: false },
    REMEDIATED: { id: { value: 'REMEDIATED' }, isFinal: false },
  },
  initialState: { value: 'COMPLIANT' },
  transitions: [
    {
      ...
      effect: {
        merge: [
          { var: 'state' },
          {
            filingHistory: {
              cat: [
                { var: 'state.filingHistory' },
                [{
                  filingId: { var: 'event.filingId' },
                  filingType: 'ANNUAL_REPORT',
                  jurisdiction: { var: 'event.jurisdiction' },
                  filedDate: { var: 'event.filedDate' },
                  periodCovered: { var: 'event.periodCovered' },
                  confirmationNumber: { var: 'event.confirmationNumber' },
                  feePaid: { var: 'event.feePaid' },
                  filedBy: { var: 'event.filedBy' },
                  documentRef: { var: 'event.documentRef' },
                }],
              ],
            },
          },
    ...
    {
      from: { value: 'COMPLIANT' },
      to: { value: 'REVIEW_PENDING' },
      eventName: 'flag_review',
      guard: { '==': [1, 1] },
      effect: {
        merge: [
          { var: 'state' },
    ...
    {
      from: { value: 'REVIEW_PENDING' },
      to: { value: 'DEFICIENT' },
      eventName: 'note_deficiency',
      guard: { '==': [1, 1] },
      effect: {
        merge: [
          { var: 'state' },
          {
            deficiencies: {
              cat: [
                { var: 'state.deficiencies' },
                [{
                  deficiencyId: { var: 'event.deficiencyId' },
                  jurisdiction: { var: 'event.jurisdiction' },
                  type: { var: 'event.type' },
                  description: { var: 'event.description' },
                  noticeDate: { var: 'event.noticeDate' },
                  cureDeadline: { var: 'event.cureDeadline' },
                  status: 'OPEN',
                }],
              ],
            },
    ...
    {
      from: { value: 'DEFICIENT' },
      to: { value: 'REMEDIATED' },
      eventName: 'cure_deficiency',
      guard: { '==': [1, 1] },
      effect: {
        merge: [
          { var: 'state' },
    ...
    {
      from: { value: 'REMEDIATED' },
      to: { value: 'COMPLIANT' },
      eventName: 'confirm_good_standing',
      guard: { '==': [1, 1] },
      effect: {
        merge: [
          { var: 'state' },
  ...
  metadata: { name: 'CorporateCompliance', version: '1.0.0' },
};

const CORPORATE_PROXY_DEFINITION = {
  states: {
    PENDING: { id: { value: 'PENDING' }, isFinal: false },
    ACTIVE: { id: { value: 'ACTIVE' }, isFinal: false },
    USED: { id: { value: 'USED' }, isFinal: true },
  ...
  transitions: [
    ...
    },
    {
      ...
      to: { value: 'REVOKED' },
      eventName: 'revoke',
      guard: { '===': [{ var: 'state.revocability.isRevocable' }, true] },
      effect: {
        merge: [
          { var: 'state' },
          { revokedAt: { var: '$timestamp' }, revokedBy: { var: 'event.agent' } },
        ],
      },
    },
  ],
  metadata: { name: 'CorporateProxy', version: '1.0.0' },
};

const CORPORATE_RESOLUTION_DEFINITION = {
  states: {
            ...
            voting: {
              merge: [
                { var: 'state.voting' },
                {
                  votes: {
                    cat: [
                      { var: 'state.voting.votes' },
                      [{
                        voterId: { var: 'event.voterId' },
                        voterName: { var: 'event.voterName' },
                        vote: { var: 'event.vote' },
                        timestamp: { var: '$timestamp' },
                      }],
                    ],
  ...
  metadata: { name: 'CorporateResolution', version: '1.0.0' },
};
